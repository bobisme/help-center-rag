...
def evaluate_all_queries(
    ...
    avg_latency = (
        sum(r["metrics"]["total_latency_ms"] for r in results) / total_queries
        if total_queries
        else 0
    )
    avg_first_stage = (
        sum(r["metrics"]["first_stage_count"] for r in results) / total_queries
        if total_queries
        else 0
    )
    avg_final_chunks = (
        sum(r["metrics"]["final_chunk_count"] for r in results) / total_queries
        if total_queries
        else 0
    )
