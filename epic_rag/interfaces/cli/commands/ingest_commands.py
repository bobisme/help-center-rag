...
def load_document_from_json(index: int) -> Document:
    ...
    try:
        ...
        if "pages" not in data or not isinstance(data["pages"], list):
            console.print(f"[bold red]Invalid format: {json_path} is not a consolidated file[/bold red]")
            raise typer.Exit(1)
            
        if index >= len(data["pages"]):
            console.print(f"[bold red]Index out of range: {index} (max: {len(data['pages'])-1})[/bold red]")
            raise typer.Exit(1)
...
@ingest_app.command("print-doc")
def print_document(
    index: int = typer.Option(..., "--index", "-i", help="Document index in epic-docs.json")
):
    ...
    console.print(Syntax(document.content, "markdown", theme="monokai", line_numbers=True))


@ingest_app.command("show-chunks")
def show_chunks(
    index: int = typer.Option(..., "--index", "-i", help="Document index in epic-docs.json"),
    with_context: bool = typer.Option(False, "--with-context", help="Show chunks with enriched context"),
    with_image_descriptions: bool = typer.Option(False, "--with-image-descriptions", help="Show chunks with image descriptions")
):
...
def run_pipeline(
    ...
    console.print(f"Document stored in SQLite database and vector embeddings stored in Qdrant.")


def register_commands(app: typer.Typer):
    """Register ingest commands with the main app."""
    app.add_typer(ingest_app, name="ingest", help="Document ingestion pipeline commands")