...
def ingest_documents(
    ...
    pattern: str = typer.Option(
        ...
        "--pattern", 
        "-p", 
        help="Pattern to match markdown files"
    ),
    ...
    min_chunk_size: int = typer.Option(
        300, 
        "--min-chunk-size", 
        help="Minimum chunk size when using dynamic chunking"
    ),
    max_chunk_size: int = typer.Option(
        800, 
        "--max-chunk-size", 
        help="Maximum chunk size when using dynamic chunking"
    ),
    ...
    # Show summary
    console.print(f"Found [bold]{len(file_paths)}[/bold] files in [cyan]{source_dir}[/cyan]")
    ...
    table.add_row("Avg Chunks Per Document", f"{total_chunks / len(results):.2f}" if results else "0")
...
def query_system(
    ...
    min_relevance_score: float = typer.Option(
        ...
        "--min-relevance", 
        "-r", 
        help="Minimum relevance score for filtering"
    ),
    ...
    if show_detailed_results:
        ...
        table.add_row("Retrieval Latency", f"{result.retrieval_latency_ms:.2f}ms" if result.retrieval_latency_ms else "N/A")
        table.add_row("Processing Latency", f"{result.processing_latency_ms:.2f}ms" if result.processing_latency_ms else "N/A")
        table.add_row("First Stage Results", str(len(result.first_stage_results.chunks)))
        table.add_row("Second Stage Results", str(len(result.second_stage_results.chunks) if result.second_stage_results else 0))
