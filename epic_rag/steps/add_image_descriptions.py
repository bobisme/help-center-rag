...
def add_image_descriptions(
    ...
    if not hasattr(image_description_service, "process_chunk_images"):
        logger.warning("Image description service does not support processing chunk images. Skipping.")
        return doc_chunks
    ...
    async def process_all_documents():
        ...
        for doc, chunks in doc_chunks:
            logger.info(f"Adding image descriptions to chunks for document: {doc.title}")
            enriched_chunks = []
            doc_total_images = 0
            ...
            for chunk in chunks:
                ...
                if matches:
                    ...
                    updated_chunk = await image_description_service.process_chunk_images(chunk)
            ...
            logger.info(f"Added descriptions for {doc_total_images} images across {len(chunks)} chunks for document: {doc.title}")
