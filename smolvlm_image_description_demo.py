#!/usr/bin/env python3
"""Demo script for testing SmolVLM image description generation."""
...
async def main():
    """Run the SmolVLM image description demo."""
    parser = argparse.ArgumentParser(
        description="Demo script for SmolVLM image description generation"
    )
    parser.add_argument(
        "--image-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Device to use (cuda, cpu, mps). If not provided, will auto-detect.",
    )
    ...
    console.print(
        Panel.fit(
            "[bold blue]SmolVLM Image Description Demo[/bold blue]\n"
    ...
    with Progress(
        ...
        task = progress.add_task(
            "Generating image descriptions with SmolVLM...", total=len(image_data)
        )
