...
class Settings:
    ...
    # Core settings
    app_name: str = "Help Center Documentation RAG"
    debug: bool = False
    ...
    def from_environment(cls) -> "Settings":
        ...
        # Core settings
        settings.app_name = os.getenv("HELP_RAG_APP_NAME", settings.app_name)
        settings.debug = os.getenv("HELP_RAG_DEBUG", "").lower() in ("true", "1", "yes")
        settings.environment = os.getenv("HELP_RAG_ENVIRONMENT", settings.environment)

        # Database settings
        settings.database.path = os.getenv("HELP_RAG_DB_PATH", settings.database.path)

        # Qdrant settings
        settings.qdrant.url = os.getenv("HELP_RAG_QDRANT_URL")
        settings.qdrant.api_key = os.getenv("HELP_RAG_QDRANT_API_KEY")
        settings.qdrant.collection_name = os.getenv(
            "HELP_RAG_QDRANT_COLLECTION", settings.qdrant.collection_name
        )

        # Embedding settings
        settings.embedding.provider = os.getenv(
            "HELP_RAG_EMBEDDING_PROVIDER", settings.embedding.provider
        )
        settings.embedding.api_key = os.getenv("HELP_RAG_EMBEDDING_API_KEY")

        # Provider-specific model settings
        settings.embedding.openai_model = os.getenv(
            "HELP_RAG_OPENAI_EMBEDDING_MODEL", settings.embedding.openai_model
        )
        settings.embedding.gemini_model = os.getenv(
            "HELP_RAG_GEMINI_EMBEDDING_MODEL", settings.embedding.gemini_model
        )

        # Cache settings
        settings.embedding.cache.enabled = os.getenv(
            "HELP_RAG_CACHE_ENABLED", "true"
        ).lower() in ("true", "1", "yes")

        if memory_size := os.getenv("HELP_RAG_CACHE_MEMORY_SIZE"):
            settings.embedding.cache.memory_size = int(memory_size)

        if expiration_days := os.getenv("HELP_RAG_CACHE_EXPIRATION_DAYS"):
            settings.embedding.cache.expiration_days = int(expiration_days)

        settings.embedding.cache.clear_on_startup = os.getenv(
            "HELP_RAG_CACHE_CLEAR_ON_STARTUP", "false"
        ).lower() in ("true", "1", "yes")

        # LLM settings
        settings.llm.provider = os.getenv(
            "HELP_RAG_LLM_PROVIDER", settings.llm.provider
        )
        settings.llm.model = os.getenv("HELP_RAG_LLM_MODEL", settings.llm.model)
        settings.llm.api_key = os.getenv("HELP_RAG_LLM_API_KEY")
