...
class SQLiteDocumentRepository(DocumentRepository):
    ...
    async def _initialize_db(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_documents_source_page_id ON documents(source_page_id);"
            )
            await db.execute(
    ...
    async def save_document(self, document: Document) -> Document:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute(
                ...
                (id, title, content, metadata, source_page_id, source_path, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    document.id,
    ...
    async def get_document(self, document_id: str) -> Optional[Document]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute(
                """
                SELECT id, title, content, metadata, source_page_id, source_path, created_at, updated_at
                FROM documents
                WHERE id = ?
            """,
                (document_id,),
    ...
    async def list_documents(
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            where_clauses = []
            if filters:
                if "source_page_id" in filters:
                    where_clauses.append("source_page_id = ?")
                    params.append(filters["source_page_id"])
    ...
    async def find_document_by_source_page_id(
        ...
        Args:
            source_page_id: The source page ID to search for

        Returns:
            Document object if found, None otherwise
        """
        logger.debug(f"Finding document by source_page_id: {source_page_id}")
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                "SELECT id FROM documents WHERE source_page_id = ?", (source_page_id,)
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
                    logger.debug(f"No document found with source_page_id: {source_page_id}")
                    return None

                logger.debug(
                    f"Found document with id: {row[0]} for source_page_id: {source_page_id}"
                )
    ...
    async def replace_document(self, document: Document) -> Document:
        ...
        Returns:
            The saved document
        """
        if not document.source_page_id:
            logger.warning("Cannot replace document without source_page_id")
            return await self.save_document(document)

        logger.info(f"Replacing document with source_page_id: {document.source_page_id}")

        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("BEGIN TRANSACTION")
            try:
                # Find existing document by source_page_id
                async with db.execute(
                    "SELECT id FROM documents WHERE source_page_id = ?",
                    (document.source_page_id,),
                ) as cursor:
                    row = await cursor.fetchone()
                    if row:
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute(
                "SELECT COUNT(*) FROM documents WHERE source_page_id IS NOT NULL"
            ) as cursor:
                row = await cursor.fetchone()
                if row:
